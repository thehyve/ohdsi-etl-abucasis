...
FROM public.tb_prestaci
  ...
  LEFT JOIN cdm5.source_to_concept_map pres ON cod_prestacion = pres.source_code AND pres.source_vocabulary_id = 'ABUCASIS_TIP_PREST'
  LEFT JOIN cdm5.source_to_concept_map unit ON cod_ud_medida = unit.source_code AND unit.source_vocabulary_id = 'ABUCASIS_UD_MEDIDAS'
WHERE public.tb_prestaci.cod_prestacion NOT LIKE '-1'
;